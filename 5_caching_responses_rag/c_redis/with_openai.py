...
REDIS_TTL_SECONDS = int(os.getenv("REDIS_TTL_SECONDS", "0"))
...
class RedisQueryCache:
    def __init__(self, redis_url: str, key_prefix: str, ttl_seconds: int = 0):
        try:
            import redis
    ...
    def get(self, key: str) -> Optional[Tuple[str, float]]:
        ...
        return str(parsed["response"]), float(parsed["original_time"])

    def set(self, key: str, response: str, original_time: float) -> None:
        cache_key = self._cache_key(key)
        payload = json.dumps({"response": response, "original_time": original_time})
...
class RAGQueryProcessor:
    def __init__(self):
        ...
        if not REDIS_CACHE_URL:
            raise ValueError("REDIS_CACHE_URL is required for redis cache example")
        ...
        self.query_cache = RedisQueryCache(REDIS_CACHE_URL, CACHE_KEY_PREFIX, REDIS_TTL_SECONDS)
